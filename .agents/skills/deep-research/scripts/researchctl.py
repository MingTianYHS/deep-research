...
def topic_dir(slug: str | None = None) -> Path:
    try:
        return resolve_topic(WORKSPACE_ROOT, slug)
...
def lock(root: Path):
    return exclusive_lock(root / ".deep-research.lock")
...
def _ensure_usage_state(state: dict[str, Any]) -> dict[str, Any]:
    value = dict(state)
    current = _usage(value.get("usage"))
    if not isinstance(value.get("lifetime_usage"), dict):
        value["lifetime_usage"] = dict(current)
    else:
        value["lifetime_usage"] = _usage(value["lifetime_usage"])
    value["usage"] = current
    return value


def _add_lifetime(state: dict[str, Any], delta: dict[str, int]) -> dict[str, Any]:
    value = _ensure_usage_state(state)
    lifetime = dict(value["lifetime_usage"])
    for key in USAGE_KEYS:
        lifetime[key] += int(delta.get(key, 0) or 0)
    value["lifetime_usage"] = lifetime
    return value
...
5. 每轮结束更新知识增量和最多五项后续调研建议，然后等待用户。

`claims.jsonl`、`evidence/cards.jsonl` 和 accepted Source Attempt 是事实记忆；`memory/current.md` 是有界导航视图，不是证据。Lite/Standard 使用一次 Critic 和机械 lineage audit；Deep 保留严格 Quote Audit，但任何 Profile 都不得自动开始下一轮。
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    for relative in ("reports", "plans", "logs", "logs/workers", "logs/critic_reviews", "logs/syntheses", "memory", "evidence"):
    ...
    for relative, content in (("questions.md", "# Research questions\n\n尚未建立当前 Research Design。\n"), ("claims.jsonl", ""), ("evidence/cards.jsonl", ""), ("logs/runs.jsonl", ""), ("logs/source_attempts.jsonl", ""), ("memory/lessons.jsonl", "")):
    ...
    next_command = f"cd {json.dumps(str(root), ensure_ascii=False)}; codex"
    print(json.dumps({"topic": slug, "workspace_root": str(WORKSPACE_ROOT), "workspace": str(root), "agent_entry": "AGENTS.md", "warning": warning, "next_command": next_command}, ensure_ascii=False, indent=2))


def cmd_plan(args: argparse.Namespace) -> None:
    root = topic_dir(args.slug)
    with lock(root):
        state = _ensure_usage_state(read_json(root / "state.json", {})); profile = state.get("budget_profile", "standard"); design_path = root / "plans/current-design.json"
        if design_path.exists() and not args.force:
            ...
            existed = design_path.exists(); operation = "reset" if existed else "created"
        ...
        state.update(status="planned", open_questions=ids, next_actions=["review synchronized design", "validate current design", "start baseline run" if not state.get("baseline_completed") else "start user-approved incremental run"])
...
def cmd_brief(args: argparse.Namespace) -> None:
    root = topic_dir(args.slug); brief = build_brief(root, args.question); brief["reuse_plan"] = build_reuse_plan(root, args.question)
    context = render_context(root, brief); memory = render_current_memory(root)
...
def cmd_run_start(args: argparse.Namespace) -> None:
    root = topic_dir(args.slug)
    with lock(root):
        state = _ensure_usage_state(read_json(root / "state.json", {}))
        ...
        state.update(status="researching", active_run_id=run_id, current_run_mode=mode, usage=_usage({})); atomic_write_json(root / "state.json", state)
        append_jsonl(root / "logs/runs.jsonl", [{"id": run_id, "mode": mode, "status": "running", "started_at": utc_now(), "run_usage": _usage({})}])
    print(json.dumps({"topic": state.get("topic"), "run_id": run_id, "mode": mode, "usage": state["usage"], "lifetime_usage": state["lifetime_usage"]}, ensure_ascii=False, indent=2))


def cmd_ingest_worker(args: argparse.Namespace) -> None:
    root = topic_dir(args.slug)
    with lock(root):
        state = _ensure_usage_state(read_json(root / "state.json", {})); profile = load_budgets()[state.get("budget_profile", "standard")]; result = json.loads(Path(args.file).read_text(encoding="utf-8"))
        ...
        outcome["budget_delta"] = total_delta; outcome["budget_verification"] = "worker_self_reported"; outcome["run_usage"] = updated["usage"]; outcome["lifetime_usage"] = updated["lifetime_usage"]
...
def cmd_run_finish(args: argparse.Namespace) -> None:
    ...
    with lock(root):
        ...
        finished_at = utc_now(); run_usage = dict(state["usage"])
        updates = {"status": args.status, "active_run_id": None, "current_run_mode": None, "last_run_at": finished_at}
        ...
        append_jsonl(root / "logs/runs.jsonl", [{"id": run_id, "status": args.status, "finished_at": finished_at, "note": args.note, "run_usage": run_usage, "lifetime_usage": state["lifetime_usage"], "completion_gates": completion}])
        memory = render_current_memory(root); backlog = load_backlog(root)
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    for relative in ("topic.toml", "state.json", "AGENTS.md", "context.md", "questions.md", "claims.jsonl", "evidence/cards.jsonl", "memory/lessons.jsonl", "logs/runs.jsonl", "logs/source_attempts.jsonl"):
        if not (root / relative).exists(): errors.append(f"missing {relative}")
    for relative in ("memory/current.md", "plans/research-backlog.json"):
        if not (root / relative).exists(): warnings.append(f"legacy workspace missing {relative}; run research.py brief to generate assistant memory")
    ...
    backlog_path = root / "plans/research-backlog.json"
    if backlog_path.exists(): errors += [f"backlog: {item}" for item in validate_next_research(load_backlog(root)["items"], set(evidence))]
...
def parser():
    ...
    command = add_topic(sub, "run-start", cmd_run_start); command.add_argument("--mode", choices=["baseline", "initial", "incremental", "deep-dive"], default="initial")
