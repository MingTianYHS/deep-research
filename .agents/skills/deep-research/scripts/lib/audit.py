...
def create_audit(report_path: Path, evidence: dict[str, dict[str, Any]], output: Path, source_attempts: dict[str, dict[str, Any]] | None = None, run_id: str | None = None) -> dict[str, Any]:
    ...
    audit = {"report": str(report_path), "report_sha256": file_sha256(report_path), "run_id": run_id, "created_at": utc_now(), "verification_mode": "strict_quote", "source_identity_frozen": True, "items": items}; atomic_write_json(output, audit); return audit
...
def mechanically_verify_audit(path: Path) -> dict[str, Any]:
    ...
    This mode verifies citation existence, accepted Source Attempt lineage, frozen
    content identity, and a non-empty Evidence quote. It intentionally records
    match_type=lineage_only so consumers can distinguish it from a Critic fetch.
    """
...
def validate_audit(path: Path, require_all_verified: bool = False) -> dict[str, Any]:
    audit = read_json(path, {}); errors, counts = [], {status: 0 for status in ALLOWED_STATUSES}; report = Path(audit.get("report", "")); mechanical = audit.get("verification_mode") == MECHANICAL_MODE
    ...
    for index, item in enumerate(items, 1):
        ...
        if status == "verified":
            ...
            if item.get("checked_by") != expected_checker: errors.append(f"item {index}: checked_by must be {expected_checker}")
            ...
            if mechanical:
                if match_type != "lineage_only": errors.append(f"item {index}: mechanical audit requires lineage_only match")
                if item.get("observed_text") != item.get("expected_quote"): errors.append(f"item {index}: mechanical audit must preserve the Evidence quote")
            else:
