...
def registry() -> dict:
    value = load_providers(PROVIDERS_FILE)
    errors = validate_providers(value)
    if errors:
        raise SystemExit("invalid providers registry: " + "; ".join(errors))
    return value
...
def cmd_providers(args: argparse.Namespace) -> None:
    ...
    if args.name:
        if args.name not in providers:
            raise SystemExit(f"unknown provider: {args.name}")
...
def cmd_cost_record(args: argparse.Namespace) -> None:
    validate_usage(registry(), args.provider, args.unit)
...
def cmd_release_check(args: argparse.Namespace) -> None:
    ...
    provider_errors = validate_providers(load_providers(PROVIDERS_FILE))
    result["errors"].extend(f"providers: {error}" for error in provider_errors)
