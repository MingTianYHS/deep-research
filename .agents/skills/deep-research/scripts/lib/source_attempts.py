...
def build_attempt(url: str, tool: str, http_status: int | None, content: str, *, source_version: str | None = None, access_mode: str = "public_static", query_id: str | None = None, discovery_method: str = "known_url", discovered_via_source_attempt_id: str | None = None) -> dict[str, Any]:
    ...
    return {"id": f"src-{uuid.uuid4().hex[:12]}", "url": url, "normalized_url": normalize_url(url), "tool": tool, "access_mode": access_mode, "http_status": http_status, "source_version": source_version, "query_id": query_id, "discovery_method": discovery_method, "discovered_via_source_attempt_id": discovered_via_source_attempt_id, **assessment}
