...
def exclusive_lock(path: Path, timeout_seconds: float = 10.0, stale_seconds: float = 300.0):
    path.parent.mkdir(parents=True, exist_ok=True); deadline = time.monotonic() + timeout_seconds; fd = None
    while fd is None:
        try:
            fd = os.open(path, os.O_CREAT | os.O_EXCL | os.O_WRONLY); os.write(fd, f"pid={os.getpid()} at={utc_now()}\n".encode())
        except FileExistsError:
            try:
                if time.time() - path.stat().st_mtime > stale_seconds: path.unlink(); continue
