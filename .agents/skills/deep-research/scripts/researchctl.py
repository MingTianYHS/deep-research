...
from lib.evidence import ingest_worker_result,preview_worker_result,validate_card
from lib.io_utils import atomic_write_json,exclusive_lock,iter_jsonl,read_json,utc_now
from lib.lifecycle import completion_check,record_critic_review,validate_provenance,validate_reflection_link,worker_results
...
def slugify(value:str)->str:value=unicodedata.normalize("NFKC",value).strip().lower();value=re.sub(r"[^\w\-\u4e00-\u9fff]+","-",value);value=re.sub(r"-+","-",value).strip("-");return safe_component(value,f"topic-{uuid.uuid4().hex[:8]}")
...
def agents_template(title:str,root:Path)->str:return f"""# {title} 主题专家协调者
...
每轮结束前必须保存 Critic Review，并让 `run-finish --status complete` 通过完成门禁。结束后提交引用该 Review 的结构化 Reflection。只有 Critic 明确接受的可复用研究经验才能进入 `memory/lessons.jsonl`。
...
def cmd_init(a):
 ...
 for rel in ["evidence/raw","reports","plans","cache","logs","logs/workers","logs/critics","memory"]:(root/rel).mkdir(parents=True,exist_ok=True)
 (root/"topic.toml").write_text(f'title = {json.dumps(a.title,ensure_ascii=False)}\nslug = {json.dumps(slug,ensure_ascii=False)}\ncreated_at = "{utc_now()}"\nstatus = "active"\nbudget_profile = "{a.budget}"\nlanguage = "zh-CN"\n\n[scope]\ninclude = []\nexclude = []\ngeographies = []\n',encoding="utf-8");(root/"AGENTS.md").write_text(agents_template(a.title,root),encoding="utf-8")
 ...
 state={"workspace_format_version":2,"topic":slug,"status":"new","knowledge_status":"empty","baseline_completed":False,"research_generation":0,"budget_profile":a.budget,"created_at":utc_now(),"last_run_at":None,"active_run_id":None,"context_generated_at":None,"usage":{"estimated_input_tokens":0,"estimated_output_tokens":0,"queries":0,"pages":0,"evidence_cards":0},"applied_worker_ingestions":[],"open_questions":[],"next_actions":["define decision context and scope","create baseline research design"]};atomic_write_json(root/"state.json",state);render_context(root,build_brief(root));warning="--install-agent is deprecated and no per-topic Agent TOML was created." if a.install_agent else None;print(json.dumps({"topic":slug,"workspace_root":str(WORKSPACE_ROOT),"workspace":str(root),"agent_entry":"AGENTS.md","warning":warning,"next_command":f"cd {json.dumps(str(root),ensure_ascii=False)}; codex"},ensure_ascii=False,indent=2))
def cmd_plan(a):
 ...
 with lock(root):
  ...
  if design_path.exists() and not a.force:
   ...
   operation="reset" if existed else "created"
...
def cmd_incremental_plan(a):root=topic_dir(a.slug);brief=build_brief(root,a.question);path=root/"plans"/f"brief-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json";atomic_write_json(path,brief);print(json.dumps({"path":str(path),"brief":brief},ensure_ascii=False,indent=2))
def cmd_brief(a):
...
def cmd_critic_record(a):
 root=topic_dir(a.slug)
 with lock(root):
  value=json.loads(Path(a.file).read_text(encoding="utf-8"));state=read_json(root/"state.json",{})
  if value.get("run_id")!=state.get("active_run_id"):raise SystemExit("critic review run_id must match the active run")
  try:outcome=record_critic_review(root,value)
  except ValueError as e:raise SystemExit(str(e)) from e
...
def cmd_reflect(a):
 ...
 with lock(root):
  ...
  try:validate_reflection_link(root,value);outcome=apply_reflection(root,value)
  except ValueError as e:raise SystemExit(str(e)) from e
 print(json.dumps(outcome,ensure_ascii=False,indent=2))
def cmd_run_start(a):
 root=topic_dir(a.slug)
...
def cmd_ingest_worker(a):
 root=topic_dir(a.slug)
 with lock(root):
  state=read_json(root/"state.json",{});active_run=state.get("active_run_id")
  if not active_run:raise SystemExit("worker ingestion requires an active run")
  profile_name=state.get("budget_profile","standard");profile=load_budgets()[profile_name];result=json.loads(Path(a.file).read_text(encoding="utf-8"))
  if result.get("budget_profile")!=profile_name:raise SystemExit("worker budget_profile does not match topic budget_profile")
  if result.get("run_id")!=active_run:raise SystemExit("worker run_id does not match the active run")
  remaining=report(state,profile)["remaining"]["evidence_cards"]
  try:plan=preview_worker_result(root/"evidence/cards.jsonl",result,remaining)
  except ValueError as e:raise SystemExit(str(e)) from e
  if not plan["already_ingested"] and len(worker_results(root,active_run))>=int(profile["max_workers"]):raise SystemExit(f"max_workers exceeded for run: {len(worker_results(root,active_run))} >= {profile['max_workers']}")
  applied=list(state.get("applied_worker_ingestions",[]));ingestion_id=plan["worker_result_id"]
  if ingestion_id not in applied:
   try:state=apply_delta(state,profile,plan["budget_delta"])
   except BudgetExceeded as e:raise SystemExit(f"budget exceeded: {e}") from e
   applied.append(ingestion_id);state["applied_worker_ingestions"]=applied;atomic_write_json(root/"state.json",state)
  try:outcome=ingest_worker_result(root/"evidence/cards.jsonl",result,remaining)
  except (OSError,ValueError) as e:raise SystemExit(f"worker ingestion incomplete; retry the same file: {e}") from e
 print(json.dumps(outcome,ensure_ascii=False,indent=2))
...
def cmd_claim_status(a):
 root=topic_dir(a.slug)
 with lock(root):value=change_status(root/"claims.jsonl",a.claim,a.status,a.reason,a.approve_core,set(evidence_map(root)))
 print(json.dumps(value,ensure_ascii=False,indent=2))
def cmd_claims(a):values=list(materialize(topic_dir(a.slug)/"claims.jsonl").values());values=[v for v in values if v.get("status")==a.status] if a.status else values;print(json.dumps({"claims":values},ensure_ascii=False,indent=2))
def cmd_report_init(a):root=topic_dir(a.slug);state=read_json(root/"state.json",{});subject=topic_title(root,a.slug or root.name);title=a.title or f"{subject}调研报告";path=Path(a.output) if a.output else root/"reports"/report_filename(subject,a.type);scaffold(path,title,a.type,materialize(root/"claims.jsonl"),state.get("last_run_at"));print(json.dumps({"report":str(path)},ensure_ascii=False,indent=2))
...
def cmd_run_finish(a):
 ...
 with lock(root):
  ...
  if a.status=="complete":
   gate=completion_check(root,rid,state.get("budget_profile","standard"))
   if not gate["valid"]:raise SystemExit("completion gate failed: "+"; ".join(gate["errors"]))
  state.update(status=a.status,active_run_id=None,last_run_at=utc_now());atomic_write_json(root/"state.json",state)
  from lib.io_utils import append_jsonl
  append_jsonl(root/"logs/runs.jsonl",[{"id":rid,"status":a.status,"finished_at":utc_now(),"note":a.note,"completion_gate":gate}])
  with (root/"logs/change_log.md").open("a",encoding="utf-8") as h:h.write(f"- {utc_now()} {rid} finished: {a.status}"+(f" — {a.note}" if a.note else "")+"\n")
 print(json.dumps({"topic":state.get("topic"),"run_id":rid,"status":a.status,"completion_gate":gate,"next_action":"create and apply a Critic-linked reflection"},ensure_ascii=False,indent=2))
def cmd_validate(a):
 ...
 if design_path.exists():
  design=read_json(design_path,{});check=validate_design(design,state.get("budget_profile","standard"));errors += [f"design: {x}" for x in check["errors"]];warnings += [f"design: {x}" for x in check["warnings"]];ids=[q.get("id") for q in design.get("questions",[]) if q.get("status","open")=="open"]
 ...
 errors += [f"claims: {x}" for x in validate_events(root/"claims.jsonl",set(evidence))];errors += [f"provenance: {x}" for x in validate_provenance(root)];errors += [f"tools: {x}" for x in validate_registry(load_registry(TOOLS_FILE))];print(json.dumps({"valid":not errors,"errors":sorted(set(errors)),"warnings":sorted(set(warnings))},ensure_ascii=False,indent=2))
 if errors:raise SystemExit(1)
def cmd_status(a):root=topic_dir(a.slug);counts={name:sum(1 for _ in iter_jsonl(root/rel)) for name,rel in [("tasks","tasks.jsonl"),("claim_events","claims.jsonl"),("evidence","evidence/cards.jsonl"),("lessons","memory/lessons.jsonl"),("run_events","logs/runs.jsonl"),("source_attempts","logs/source_attempts.jsonl")]};counts["claims"]=len(materialize(root/"claims.jsonl"));print(json.dumps({"workspace":str(root),"state":read_json(root/"state.json",{}),"counts":counts},ensure_ascii=False,indent=2))
def cmd_budget(a):state=read_json(topic_dir(a.slug)/"state.json",{});name=state.get("budget_profile","standard");print(json.dumps({"profile":name,**report(state,load_budgets()[name])},ensure_ascii=False,indent=2))
def cmd_tools(a):
...
def cmd_estimate(a):text=Path(a.file).read_text(encoding="utf-8") if a.file else a.text;print(json.dumps({"characters":len(text),"estimated_tokens":math.ceil(len(text)/3.2)},indent=2))
def add_topic(sub,name,func):x=sub.add_parser(name);x.add_argument("slug",nargs="?");x.set_defaults(func=func);return x
def parser():
 ...
 x=add_topic(s,"plan",cmd_plan);x.add_argument("--questions",type=int,default=5,choices=range(1,9));x.add_argument("--force",action="store_true")
 x=add_topic(s,"incremental-plan",cmd_incremental_plan);x.add_argument("--question");x=add_topic(s,"brief",cmd_brief);x.add_argument("--question");x.add_argument("--output");x=add_topic(s,"critic-record",cmd_critic_record);x.add_argument("--file",required=True);x=add_topic(s,"reflect",cmd_reflect);x.add_argument("--file",required=True);x=add_topic(s,"run-start",cmd_run_start);x.add_argument("--mode",choices=["baseline","initial","incremental","deep-dive"],default="initial")
 x=add_topic(s,"record-usage",cmd_record_usage);x.add_argument("--queries",type=int,default=0);x.add_argument("--pages",type=int,default=0);x.add_argument("--evidence-cards",type=int,default=0);x.add_argument("--input-tokens",type=int,default=0);x.add_argument("--output-tokens",type=int,default=0);x.add_argument("--force",action="store_true");x=add_topic(s,"ingest-worker",cmd_ingest_worker);x.add_argument("--file",required=True)
 x=add_topic(s,"claim-create",cmd_claim_create);x.add_argument("--text",required=True);x.add_argument("--confidence",type=float,default=.5);x.add_argument("--core",action="store_true");x=add_topic(s,"claim-link",cmd_claim_link);x.add_argument("--claim",required=True);x.add_argument("--evidence",required=True);x.add_argument("--stance",choices=["support","contradict","context"],required=True);x.add_argument("--strength",type=float,default=.5);x=add_topic(s,"claim-status",cmd_claim_status);x.add_argument("--claim",required=True);x.add_argument("--status",choices=["draft","supported","contested","rejected","unresolved"],required=True);x.add_argument("--reason",default="");x.add_argument("--approve-core",action="store_true");x=add_topic(s,"claims",cmd_claims);x.add_argument("--status")
 x=add_topic(s,"report-init",cmd_report_init);x.add_argument("--type",choices=["initial","update","final"],default="initial");x.add_argument("--title");x.add_argument("--output");x=add_topic(s,"verify-citations",cmd_verify_citations);x.add_argument("--report",required=True);x=add_topic(s,"run-finish",cmd_run_finish);x.add_argument("--status",choices=["complete","partial","failed"],default="complete");x.add_argument("--note",default="");add_topic(s,"validate",cmd_validate);add_topic(s,"status",cmd_status);add_topic(s,"budget",cmd_budget)
 x=s.add_parser("tools");x.add_argument("capability");x.add_argument("--all",action="store_true");x.set_defaults(func=cmd_tools);x=s.add_parser("estimate");g=x.add_mutually_exclusive_group(required=True);g.add_argument("--text");g.add_argument("--file");x.set_defaults(func=cmd_estimate);return p
