...
def validate_design(design: dict[str, Any], profile: str | None = None) -> dict[str, Any]:
    ...
    if not isinstance(questions, list) or not questions:
        return {
            "valid": False,
            "errors": ["questions must be a non-empty list"],
            "warnings": [],
            "question_count": 0,
            "parallel_groups": [],
        }
    ...
    if len(questions) > maximum:
        errors.append(f"profile {profile or 'deep'} allows at most {maximum} research questions")
    ...
    for index, item in enumerate(questions, 1):
        ...
        if missing:
            errors.append(f"question {index}: missing {missing}")
        if item.get("type") not in QUESTION_TYPES:
            errors.append(f"question {index}: invalid type {item.get('type')}")
        if item.get("status", "open") not in QUESTION_STATUSES:
            errors.append(f"question {index}: invalid status {item.get('status')}")
        criteria = item.get("acceptance_criteria")
        if not isinstance(criteria, list) or not criteria:
            errors.append(f"question {index}: acceptance_criteria must be a non-empty list")
        dependencies = item.get("dependencies", [])
        if not isinstance(dependencies, list):
            errors.append(f"question {index}: dependencies must be a list")
        else:
            unknown = sorted(set(dependencies) - known)
            if unknown:
                errors.append(f"question {index}: unknown dependencies {unknown}")
            if item.get("id") in dependencies:
                errors.append(f"question {index}: self dependency")
        if overlap[item.get("overlap_key")] > 1:
            errors.append(f"question {index}: duplicate overlap_key {item.get('overlap_key')}")
        ...
        if worker_profile not in WORKER_PROFILES:
            errors.append(f"question {index}: invalid worker_budget_profile {worker_profile}")
        if item.get("version_sensitive"):
            ...
            if item.get("target_version") in {"main", "latest"}:
                warnings.append(
                    f"question {index}: target_version should identify an installed or released version, not {item.get('target_version')}"
                )
    ...
    return {
        ...
        "parallel_groups": parallel_groups(questions),
    }
...
def parallel_groups(questions: list[dict[str, Any]]) -> list[list[str]]:
    remaining = {
        ...
        for item in questions
        if isinstance(item, dict) and item.get("id") and item.get("status", "open") == "open"
    }
    completed: set[str] = {
        item["id"]
        for item in questions
...
def template(title: str, question_count: int = 1, worker_budget_profile: str = "standard") -> dict[str, Any]:
    if worker_budget_profile not in WORKER_PROFILES:
        raise ValueError(f"invalid worker budget profile: {worker_budget_profile}")
    maximum = PROFILE_QUESTION_LIMITS[worker_budget_profile]
    if not 1 <= question_count <= maximum:
        raise ValueError(f"profile {worker_budget_profile} allows 1-{maximum} research questions")
    questions = []
        ...
        questions.append(
            {
                "id": f"q-{index:03d}",
                "status": "open",
                "question": f"Replace with answerable research question {index}",
                "type": "fact",
                "decision_relevance": "Why the answer matters",
                "dependencies": [],
                "overlap_key": f"unique-subtopic-{index}",
                "preferred_source_types": ["official", "paper"],
                "acceptance_criteria": [
                    "At least one primary source",
                    "Independent corroboration for a core claim",
                ],
                "disconfirming_query": "Search terms intended to disprove the expected answer",
                ...
                "worker_budget_profile": worker_budget_profile,
            }
        )
    return {
        "title": title,
        "decision_context": "What decision or understanding should this research support?",
...
def render_questions(design: dict[str, Any]) -> str:
    ...
    for item in design.get("questions", []):
        lines += [
            ...
            f"- Decision relevance: {item.get('decision_relevance')}",
            f"- Overlap key: {item.get('overlap_key')}",
            "",
        ]
    return "\n".join(lines).rstrip() + "\n"
