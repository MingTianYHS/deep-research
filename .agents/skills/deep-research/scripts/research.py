...
def cmd_validate(args: argparse.Namespace) -> None:
    topicctl.cmd_validate(argparse.Namespace(topic=args.topic, allow_language_mismatch=args.allow_language_mismatch))
...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(prog="research", description="Safe public workflow for one persistent deep-research topic.")
    ...
    _topic_argument(plan); plan.add_argument("--questions", type=int, default=5, choices=range(1, 9)); plan.set_defaults(func=cmd_plan)
    brief = sub.add_parser("brief", help="Build bounded context for the topic or one question.")
    _topic_argument(brief); brief.add_argument("--question"); brief.add_argument("--output"); brief.set_defaults(func=cmd_brief)
    start = sub.add_parser("start", help="Start a baseline, incremental, or deep-dive Run.")
    _topic_argument(start); start.add_argument("--mode", choices=["baseline", "initial", "incremental", "deep-dive"], default="initial"); start.set_defaults(func=cmd_start)
    status = sub.add_parser("status", help="Show topic state and record counts.")
    _topic_argument(status); status.set_defaults(func=cmd_status)
    report = sub.add_parser("report", help="Create a report inside the canonical topic workspace.")
    _topic_argument(report); report.add_argument("--type", choices=["initial", "update", "final"], default="initial"); report.add_argument("--title"); report.add_argument("--output"); report.add_argument("--allow-language-mismatch", action="store_true"); report.set_defaults(func=cmd_report)
    finish = sub.add_parser("finish", help="Close the active Run after all required gates.")
    _topic_argument(finish); finish.add_argument("--status", choices=["complete", "partial", "failed"], default="complete"); finish.add_argument("--note", default=""); finish.set_defaults(func=cmd_finish)
    validate = sub.add_parser("validate", help="Validate naming, workspace, design, and records.")
    _topic_argument(validate); validate.add_argument("--allow-language-mismatch", action="store_true"); validate.set_defaults(func=cmd_validate)
    return value
