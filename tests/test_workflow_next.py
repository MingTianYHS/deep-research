...
def topic(tmp_path: Path, *, active_run_id=None) -> Path:
    ...
    write_json(root / "state.json", {"topic": "测试主题", "budget_profile": "standard", "active_run_id": active_run_id, "baseline_completed": False, "open_questions": ["q-001"]})
    return root
...
def test_next_delegates_complete_assignment_to_named_researcher(tmp_path):
    ...
    assert assignment["research_context"]["recommended_action"] == "targeted_discovery"
    assert result["requires_user_input"] is False
...
def test_next_blocks_unsafe_reflection_without_critic(tmp_path):
    root = topic(tmp_path)
    write_json(root / "plans/current-design.json", design())
    (root / "logs/runs.jsonl").write_text(json.dumps({"id": "run-finished", "status": "partial", "finished_at": "2026-08-06T00:00:00Z"}) + "\n", encoding="utf-8")
    result = derive_workflow(root, SCRIPT_DIR.parent)
...
def prepare_completed_worker(root: Path):
    write_json(root / "logs/workers/worker-current.json", {"worker_result_id": "worker-current", "run_id": "run-current", "question_id": "q-001", "status": "complete", "coverage_status": "sufficient", "ingest_summary": {"accepted_evidence_ids": ["ev-current"]}})


def patch_claim_and_approval(monkeypatch):
    monkeypatch.setattr(workflow, "materialize", lambda _path: {"claim-current": {"id": "claim-current", "relations": [{"evidence_id": "ev-current", "stance": "support"}]}})
    monkeypatch.setattr(workflow, "approved_reviews_for_run", lambda _root, _run, current_only=True: [{"id": "critic-1", "status": "approved", "reviewed_snapshot": {}}])


def test_next_initializes_audit_only_for_report_citing_current_run(monkeypatch, tmp_path):
    root = topic(tmp_path, active_run_id="run-current")
    write_json(root / "plans/current-design.json", design())
    prepare_completed_worker(root)
    patch_claim_and_approval(monkeypatch)
    report = root / "reports/最终报告.md"
    report.write_text("---\ntitle: 最终报告\nstatus: complete\n---\n\n## 核心结论\n\n本轮证据支持该结论。[[ev-current]]", encoding="utf-8")
    result = derive_workflow(root, SCRIPT_DIR.parent)
...
def test_next_routes_historical_report_to_new_scaffold(monkeypatch, tmp_path):
    ...
    prepare_completed_worker(root)
    patch_claim_and_approval(monkeypatch)
    (root / "logs/runs.jsonl").write_text(json.dumps({"id": "run-current", "status": "running", "started_at": "2999-01-01T00:00:00Z"}) + "\n", encoding="utf-8")
    (root / "reports/历史报告.md").write_text("---\ntitle: 历史报告\nstatus: complete\n---\n\n## 核心结论\n\n历史结论。[[ev-historical]]", encoding="utf-8")
    result = derive_workflow(root, SCRIPT_DIR.parent)
    assert result["phase"] == "synthesis"
    assert result["next_action"] == "create_report_scaffold"


def test_next_routes_current_changes_required_to_targeted_research(monkeypatch, tmp_path):
    root = topic(tmp_path, active_run_id="run-current")
    write_json(root / "plans/current-design.json", design())
    prepare_completed_worker(root)
    monkeypatch.setattr(workflow, "materialize", lambda _path: {"claim-current": {"id": "claim-current", "relations": [{"evidence_id": "ev-current"}]}})
    monkeypatch.setattr(workflow, "approved_reviews_for_run", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(workflow, "latest_review_for_run", lambda *_args: {"id": "critic-1", "status": "changes_required", "findings": [], "targeted_searches": [{"id": "targeted-1", "finding_id": "finding-1", "question_id": "q-001", "query": "site:example.gov primary filing", "intent": "primary_source", "required_evidence": "Primary filing", "stop_condition": "Found or official archive exhausted"}]})
