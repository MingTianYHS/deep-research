...
def test_citation_verification_and_zero_citation_failure(tmp_path):
    report=tmp_path/"report.md"; report.write_text("A fact [[ev-1]] and missing [[ev-404]]."); result=verify_report(report,{"ev-1":sample_card()}); assert result["missing_evidence"]==["ev-404"] and not result["valid"]
    report.write_text("No citations."); assert not verify_report(report,{"ev-1":sample_card()})["valid"]
...
def test_report_scaffold_includes_evidence_marker(tmp_path):
    report=tmp_path/"r.md"; scaffold(report,"Research report","initial",{"cl-1":{"id":"cl-1","text":"Supported","status":"supported","relations":[{"evidence_id":"ev-1","stance":"support"}]}},None); assert "[[ev-1]]" in report.read_text()
