...
def test_init_creates_lean_topic_workspace(monkeypatch,tmp_path):
    root=init(module,tmp_path);state=json.loads((root/"state.json").read_text(encoding="utf-8"));assert state["workspace_format_version"]==2;assert state["usage"]=={"queries":0,"pages":0,"evidence_cards":0};assert state["context_generated_at"];assert (root/"AGENTS.md").exists();assert (root/"context.md").exists();assert not (root/"tasks.jsonl").exists();assert not (root/"source_map.md").exists();assert not (root/"evidence/raw").exists();assert not (root/"cache").exists();monkeypatch.chdir(root);assert module.topic_dir()==root.resolve()
...
def test_legacy_cli_does_not_expose_removed_or_guarded_commands():
