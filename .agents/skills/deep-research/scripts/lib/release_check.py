...
def check_repo(repo_root:Path)->dict[str,Any]:
 ...
 if skill_md.exists():
  ...
  if not skill_text.startswith("---\n") or "\nname: deep-research\n" not in skill_text or "\ndescription:" not in skill_text:errors.append("SKILL.md frontmatter is invalid or incomplete")
  if len(skill_text.splitlines())>500:errors.append("SKILL.md exceeds 500 lines")
 project,skill_release=project_version(repo_root),skill_version(skill_text)
 if not project or not skill_release:errors.append("project and skill versions are required")
 elif project!=skill_release:errors.append(f"version mismatch: project={project}, skill={skill_release}")
 ...
 for scan_root in (skill,agents):
  ...
  for path in scan_root.rglob("*"):
   ...
   for pattern in SECRET_PATTERNS:
    if pattern.search(text):errors.append(f"possible secret in {path.relative_to(repo_root)}")
 if not (repo_root/"tests/test_lifecycle_integration.py").exists():errors.append("full lifecycle integration test missing")
 if not (repo_root/"CHANGELOG.md").exists():warnings.append("CHANGELOG.md missing")
 if not (repo_root/".github/workflows/ci.yml").exists():warnings.append("CI workflow missing")
 return {"valid":not errors,"version":project,"errors":sorted(set(errors)),"warnings":sorted(set(warnings))}
