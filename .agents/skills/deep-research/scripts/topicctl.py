...
def _fail_naming(title: str, directory_name: str, allow_language_mismatch: bool) -> None:
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    researchctl.cmd_init(argparse.Namespace(title=args.title, slug=directory_name, budget=args.budget, force=args.force, install_agent=False))
...
def cmd_report(args: argparse.Namespace) -> None:
    ...
    if args.output:
        ...
        if not is_within(root, output):
            raise SystemExit("report output must stay inside the canonical topic workspace; use releasectl.py export-topic for external copies")
    researchctl.cmd_report_init(argparse.Namespace(slug=str(root), type=args.type, title=args.title, output=args.output))
...
def cmd_validate(args: argparse.Namespace) -> None:
    try:
        ...
        raise SystemExit(str(exc)) from exc
    subject = topic_title(root, root.name)
    errors = validate_topic_naming(subject, root.name, allow_language_mismatch=args.allow_language_mismatch)
    result = {"valid": not errors, "topic": subject, "workspace": str(root), "errors": errors}
...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(prog="topicctl")
    sub = value.add_subparsers(dest="command", required=True)
    init = sub.add_parser("init-topic")
    init.add_argument("title")
    init.add_argument("--directory-name")
    init.add_argument("--budget", choices=["lite", "standard", "deep"], default="standard")
