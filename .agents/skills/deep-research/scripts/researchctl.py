...
def topic_dir(slug: str) -> Path:
    ...
    if not path.exists(): raise SystemExit(f"topic not found: {slug} (workspace root: {WORKSPACE_ROOT})")
    return path
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    for relative in ["evidence/raw", "reports", "plans", "cache", "logs"]: (root / relative).mkdir(parents=True, exist_ok=True)
    ...
    agent = install_topic_agent(args.title, slug, args.budget, root) if args.install_agent else None
    print(json.dumps({"topic": slug, "workspace_root": str(WORKSPACE_ROOT), "workspace": str(root), "agent": str(agent) if agent else None}, ensure_ascii=False, indent=2))


def cmd_plan(args):
    root = topic_dir(args.slug); state = read_json(root / "state.json", {}); ids, lines = [], ["# Research questions", ""]
    for index in range(1, args.questions + 1):
        qid = f"q-{index:03d}"; ids.append(qid); lines += [f"## {qid}", "", "- Status: open", "- Priority: medium", "- Question: TODO", ""]
    ...
    print(json.dumps({"topic": args.slug, "questions_created": len(ids)}, ensure_ascii=False, indent=2))


def cmd_incremental_plan(args):
    root = topic_dir(args.slug); state = read_json(root / "state.json", {}); plan = build_plan(state, materialize(root / "claims.jsonl"), root / "evidence/cards.jsonl")
    path = root / "plans" / f"incremental-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"; atomic_write_json(path, plan)
...
def cmd_run_start(args):
    ...
    run_id = f"run-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}-{uuid.uuid4().hex[:6]}"; state.update(status="researching", active_run_id=run_id); atomic_write_json(root / "state.json", state)
    append_jsonl(root / "logs/runs.jsonl", [{"id": run_id, "mode": args.mode, "status": "running", "started_at": utc_now()}]); print(json.dumps({"topic": args.slug, "run_id": run_id, "mode": args.mode}, ensure_ascii=False, indent=2))


def cmd_record_usage(args):
    root = topic_dir(args.slug); state = read_json(root / "state.json", {}); profile = load_budgets()[state.get("budget_profile", "standard")]
    ...
    atomic_write_json(root / "state.json", updated); print(json.dumps(report(updated, profile), ensure_ascii=False, indent=2))


def cmd_ingest_worker(args):
    root = topic_dir(args.slug); state = read_json(root / "state.json", {}); profile = load_budgets()[state.get("budget_profile", "standard")]; remaining = report(state, profile)["remaining"]["evidence_cards"]
    result = json.loads(Path(args.file).read_text(encoding="utf-8")); outcome = ingest_worker_result(root / "evidence/cards.jsonl", result, remaining); atomic_write_json(root / "state.json", apply_delta(state, profile, {"evidence_cards": outcome["accepted"]})); print(json.dumps(outcome, ensure_ascii=False, indent=2))


def cmd_claim_create(args): print(json.dumps(create_claim(topic_dir(args.slug) / "claims.jsonl", args.text, args.confidence, args.core), ensure_ascii=False, indent=2))
...
def cmd_claim_link(args):
    ...
    print(json.dumps(link_claim(root / "claims.jsonl", args.claim, args.evidence, args.stance, args.strength), ensure_ascii=False, indent=2))


def cmd_claim_status(args): print(json.dumps(change_status(topic_dir(args.slug) / "claims.jsonl", args.claim, args.status, args.reason, args.approve_core), ensure_ascii=False, indent=2))
...
def cmd_report_init(args):
    root = topic_dir(args.slug); state = read_json(root / "state.json", {}); subject = topic_title(root, args.slug); title = args.title or f"{subject}调研报告"
    path = Path(args.output) if args.output else root / "reports" / report_filename(subject, args.type)
...
def cmd_run_finish(args):
    ...
    state.update(status=args.status, active_run_id=None, last_run_at=utc_now()); atomic_write_json(root / "state.json", state); append_jsonl(root / "logs/runs.jsonl", [{"id": run_id, "status": args.status, "finished_at": utc_now(), "note": args.note}])
    with (root / "logs/change_log.md").open("a", encoding="utf-8") as handle: handle.write(f"- {utc_now()} {run_id} finished: {args.status}" + (f" — {args.note}" if args.note else "") + "\n")
    print(json.dumps({"topic": args.slug, "run_id": run_id, "status": args.status}, ensure_ascii=False, indent=2))
...
def cmd_validate(args):
    ...
    errors.extend(f"claims: {item}" for item in validate_events(root / "claims.jsonl", set(evidence))); errors.extend(f"tools: {item}" for item in validate_registry(load_registry(TOOLS_FILE)))
    print(json.dumps({"valid": not errors, "errors": errors}, ensure_ascii=False, indent=2))
    if errors: raise SystemExit(1)


def cmd_status(args):
    root = topic_dir(args.slug); counts = {name: sum(1 for _ in iter_jsonl(root / rel)) for name, rel in [("tasks", "tasks.jsonl"), ("claim_events", "claims.jsonl"), ("evidence", "evidence/cards.jsonl"), ("run_events", "logs/runs.jsonl")]}; counts["claims"] = len(materialize(root / "claims.jsonl")); print(json.dumps({"workspace": str(root), "state": read_json(root / "state.json", {}), "counts": counts}, ensure_ascii=False, indent=2))


def cmd_budget(args):
    state = read_json(topic_dir(args.slug) / "state.json", {}); name = state.get("budget_profile", "standard"); print(json.dumps({"profile": name, **report(state, load_budgets()[name])}, ensure_ascii=False, indent=2))
...
def cmd_estimate(args):
    text = Path(args.file).read_text(encoding="utf-8") if args.file else args.text; print(json.dumps({"characters": len(text), "estimated_tokens": math.ceil(len(text) / 3.2)}, indent=2))
