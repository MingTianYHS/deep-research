...
def _agent_check(path: Path, expected_name: str) -> list[dict[str, Any]]:
    ...
    if missing:
        return [
            _item("error", "agent_contract", f"Agent missing fields: {missing}", path)
        ]
    if value.get("name") != expected_name:
        return [
            _item(
                "error",
                "agent_name",
                f"Expected agent name {expected_name}, got {value.get('name')}",
                path,
            )
        ]
    if value.get("sandbox_mode") != "read-only":
        return [
            _item(
                "error",
                "agent_sandbox",
                "Research agents must use read-only sandbox mode.",
                path,
            )
        ]
...
def diagnose(
    ...
    *,
    ...
    checks.append(
        _item(
            "ok" if skill_dir == expected_skill else "error",
            "user_skill_layout" if skill_dir == expected_skill else "skill_not_user_level",
            "User-level Skill location is active."
            if skill_dir == expected_skill
            else "Only the user-level Skill layout is supported.",
            skill_dir if skill_dir == expected_skill else expected_skill,
        )
    )
    checks.append(
        _item(
            "ok" if version >= (3, 11) else "error",
            "python_version",
    ...
    for relative in REQUIRED_SKILL_FILES:
        ...
        checks.append(
            _item(
                ...
                "skill_file",
                f"Required Skill file: {relative}",
                path,
            )
        )
    ...
    if workspace_root.exists() and workspace_root.is_dir():
        ...
        try:
            ...
            checks.append(
                _item(
                    "ok",
                    "workspace_writable",
                    "Workspace write probe succeeded.",
                    workspace_root,
                )
            )
            ...
            checks.append(
                _item(
                    "error",
                    "workspace_writable",
                    f"Workspace write probe failed: {exc}",
                    workspace_root,
                )
            )
    else:
        ...
        checks.append(
            _item(
                "warning" if creatable else "error",
                "workspace_missing",
                "Workspace does not exist yet; it can be created."
                if creatable
                else "Workspace and its writable parent are missing.",
                workspace_root,
            )
        )
    ...
    checks.append(
        _item(
            "ok" if web_access.is_file() else "warning",
            "web_access",
            "web-access is available for authorized login/anti-bot fallback."
            if web_access.is_file()
            else "Optional web-access Skill not found; login/anti-bot pages may remain unavailable.",
            web_access,
        )
    )
    ...
    if global_agents.is_file() and global_agents.stat().st_size > 20_000:
        checks.append(
            _item(
                "warning",
                "large_global_instructions",
                "Global AGENTS.md exceeds 20 KB and may inflate every worker context.",
                global_agents,
            )
        )
    ...
    return {
        ...
        "web_access_skill": str(web_access),
        "checks": checks,
