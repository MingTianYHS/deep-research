...
def _historical_query_keys(topic_root: Path, active_run_id: str | None) -> set[tuple[str, str]]:
    ...
    for path in directory.glob("*.json"):
        ...
        for query in worker.get("queries_run", []) if isinstance(worker.get("queries_run"), list) else []:
            if isinstance(query, dict) and _normalized_query(query.get("query")):
                values.add((_normalized_query(query.get("query")), str(query.get("intent") or "")))
    return values
...
def validate_ingest_context(topic_root: Path, result: dict[str, Any], *, allow_existing_worker: bool = False) -> dict[str, Any]:
    ...
    design_path = topic_root / "plans/current-design.json"; design = read_json(design_path, {})
    question_id = result.get("question_id"); question = None
    if not design_path.is_file() or not isinstance(design, dict): errors.append("worker ingestion requires plans/current-design.json")
        ...
        if not question: errors.append(f"worker question_id {question_id} is not in the current Research Design")
            ...
            if result.get("budget_profile") != question_profile: errors.append("worker budget_profile does not match the assigned question")
    ...
    evidence = {str(item.get("id")): item for _, item in iter_jsonl(topic_root / "evidence/cards.jsonl") if item.get("id")}
    ...
    if isinstance(reused, list):
        for evidence_id in reused:
            ...
            elif card.get("question_id") != question_id: errors.append(f"reused Evidence belongs to another question: {evidence_id}")
    ...
    for index, query in enumerate(result.get("queries_run", []) if isinstance(result.get("queries_run"), list) else [], 1):
        ...
        if key in historical and query.get("repeat_reason") not in REPEAT_REASONS:
            errors.append(f"queries_run[{index}] repeats a historical query without an allowed repeat_reason")
