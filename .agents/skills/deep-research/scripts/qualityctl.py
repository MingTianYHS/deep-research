...
def root_for(slug: str) -> Path:
    ...
    if not root.exists():
        raise SystemExit(f"topic not found: {slug} (workspace root: {WORKSPACE_ROOT})")
    return root
...
def cmd_quality(args: argparse.Namespace) -> None:
    ...
    policy = load_policy(POLICY_FILE); gates = policy["quality_gates"]
    result["question_count"] = len(question_ids); result["question_coverage"] = round(coverage, 4)
