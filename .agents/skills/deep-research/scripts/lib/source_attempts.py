...
ERROR_MARKERS = re.compile(r"(?is)<title>\s*(?:403|404)[^<]*</title>|\b403\s*:\s*forbidden\b|\b404\s*:\s*not found\b|\bcrawl_livecrawl_timeout\b")
...
def may_attempt(path: Path, url: str, max_attempts: int = 2) -> dict[str, Any]:
    ...
    accepted = next((item for item in attempts if item.get("status") == "accepted"), None)
    if accepted:
...
def build_attempt(url: str, tool: str, http_status: int | None, content: str, *, source_version: str | None = None) -> dict[str, Any]:
    assessment = assess_response(http_status, content)
    return {"url": url, "normalized_url": normalize_url(url), "tool": tool, "http_status": http_status, "source_version": source_version, **assessment}
...
def append_attempt(path: Path, attempt: dict[str, Any]) -> dict[str, Any]:
    ...
    if content_hash:
        ...
        if duplicate:
            attempt = {**attempt, "duplicate_content_of": duplicate.get("normalized_url"), "independent_origin": False}
        else:
            attempt = {**attempt, "independent_origin": True}
