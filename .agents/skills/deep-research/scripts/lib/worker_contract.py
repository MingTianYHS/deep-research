...
def validate_worker_result(result: dict[str, Any], limits: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(result.get("suggested_followups"), list) and len(result["suggested_followups"]) > 2: errors.append("suggested_followups must contain at most 2 items")
    attempts = _validate_attempts(result, errors); _validate_cards(result, attempts, errors)
    ...
    if not isinstance(usage, dict): errors.append("budget_used must be an object")
        ...
        for usage_key, limit_key in USAGE_LIMITS.items():
            ...
            if not isinstance(value, (int, float)) or isinstance(value, bool) or value < 0: errors.append(f"budget_used.{usage_key} must be non-negative"); continue
            if value > limits[limit_key]: errors.append(f"budget_used.{usage_key} {value} exceeds {limit_key} {limits[limit_key]}")
        same_url = usage.get("same_url_attempts_max")
        if not isinstance(same_url, (int, float)) or isinstance(same_url, bool) or same_url < 0: errors.append("budget_used.same_url_attempts_max must be non-negative")
        elif same_url > limits["max_same_url_attempts"]: errors.append("same URL retry limit exceeded")
        reserve = usage.get("output_reserve_ratio")
        if not isinstance(reserve, (int, float)) or isinstance(reserve, bool): errors.append("budget_used.output_reserve_ratio is required")
        elif reserve < limits["reserve_output_ratio"]: errors.append("output reserve ratio was not honored")
