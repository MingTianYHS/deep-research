...
def diagnose(skill_dir: Path, workspace_root: Path, *, home: Path | None = None, python_version: tuple[int, int] | None = None) -> dict[str, Any]:
    ...
    web_access = expected_web_access_dir(home) / "SKILL.md"; checks.append(_item("ok" if web_access.is_file() else "warning", "web_access", "web-access is available for authorized login/anti-bot fallback." if web_access.is_file() else "Optional web-access Skill not found; login/anti-bot pages may remain unavailable.", web_access))
    global_agents = codex_home(home) / "AGENTS.md"
    if global_agents.is_file() and global_agents.stat().st_size > 20_000: checks.append(_item("warning", "large_global_instructions", "Global AGENTS.md exceeds 20 KB and may inflate every worker context.", global_agents))
    errors = [item for item in checks if item["level"] == "error"]; warnings = [item for item in checks if item["level"] == "warning"]
