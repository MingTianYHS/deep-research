...
def _quality(root: Path, cards: list[dict[str, Any]], policy_file: Path) -> dict[str, Any]:
    policy = load_policy(policy_file); result = evaluate(cards, policy, date.today())
    design = read_json(root / "plans/current-design.json", {}); questions = [item for item in design.get("questions", []) if isinstance(item, dict) and item.get("id")]
    question_ids = {item["id"] for item in questions}; covered = {item.get("question_id") for item in cards if item.get("question_id") in question_ids and item.get("prompt_injection_risk") != "high"}
    coverage = len(covered) / len(question_ids) if question_ids else 0.0; gates = policy["quality_gates"]
