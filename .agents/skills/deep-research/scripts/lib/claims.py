...
def create(path: Path, text: str, confidence: float, is_core: bool) -> dict[str, Any]:
    if not text.strip(): raise ValueError("claim text must not be empty")
    if not 0.0 <= confidence <= 1.0: raise ValueError("claim confidence must be between 0 and 1")
    claim = {"id": f"cl-{uuid.uuid4().hex[:12]}", "text": text.strip(), "status": "draft", "confidence": confidence, "is_core": is_core, "relations": [], "created_at": utc_now(), "updated_at": utc_now()}
    append_jsonl(path, [{"type": "claim.created", "claim_id": claim["id"], "claim": claim, "at": utc_now()}])
    return claim


def link(path: Path, claim_id: str, evidence_id: str, stance: str, strength: float) -> dict[str, Any]:
    if claim_id not in materialize(path): raise ValueError(f"unknown claim: {claim_id}")
    if stance not in STANCES: raise ValueError(f"invalid relation stance: {stance}")
    if not 0.0 <= strength <= 1.0: raise ValueError("relation strength must be between 0 and 1")
...
def change_status(path: Path, claim_id: str, target: str, reason: str, approve_core: bool) -> dict[str, Any]:
    claim = materialize(path).get(claim_id)
    if not claim: raise ValueError(f"unknown claim: {claim_id}")
    if target not in STATUSES: raise ValueError(f"invalid claim status: {target}")
    if claim["status"] == target: return {"type": "claim.status.noop", "claim_id": claim_id, "to": target}
    ...
    try:
        for number, event in iter_jsonl(path):
            ...
            if event_type == "claim.created":
                ...
                if not claim.get("id") or not claim.get("text"): errors.append(f"line {number}: invalid claim.created")
                else: known.add(claim["id"])
            ...
            if event_type == "claim.relation.added":
                ...
                if evidence_id not in evidence_ids: errors.append(f"line {number}: unknown evidence {evidence_id}")
