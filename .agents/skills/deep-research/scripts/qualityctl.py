...
from lib.research_memory import latest_verifications
...
def evidence_map(root: Path) -> dict[str, dict]:
...
def cmd_quality(args: argparse.Namespace) -> None:
    ...
    question_ids = set(re.findall(r"^##\s+(q-[A-Za-z0-9_-]+)", (root / "questions.md").read_text(encoding="utf-8"), re.MULTILINE))
    coverage = result["questions_covered"] / len(question_ids) if question_ids else 0.0
    ...
    if args.require_gates and not result["passes_all_gates"]:
        raise SystemExit(1)
...
def _create_effective_audit(root: Path, report_path: Path, output: Path, run_id: str | None) -> dict:
    return create_audit(report_path, evidence_map(root), output, source_attempt_map(root), run_id=run_id, verifications=latest_verifications(root))


def cmd_audit_init(args: argparse.Namespace) -> None:
    root, report_path, output, run_id = _audit_paths(args)
    audit = _create_effective_audit(root, report_path, output, run_id)
    print(json.dumps({"audit": str(output), "run_id": run_id, "items": len(audit["items"]), "verification_mode": audit["verification_mode"]}, ensure_ascii=False, indent=2))


def cmd_audit_mechanical(args: argparse.Namespace) -> None:
    root, report_path, output, run_id = _audit_paths(args)
    _create_effective_audit(root, report_path, output, run_id)
...
def cmd_audit_validate(args: argparse.Namespace) -> None:
    result = validate_audit(Path(args.audit), require_all_verified=args.final)
    print(json.dumps(result, ensure_ascii=False, indent=2))
    if not result["valid"]:
        raise SystemExit(1)
...
def _report_arguments(parser: argparse.ArgumentParser) -> None:
    _topic_argument(parser)
    parser.add_argument("--report", required=True)
    parser.add_argument("--output")


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(prog="qualityctl")
    sub = value.add_subparsers(dest="command", required=True)
    quality = sub.add_parser("quality-report")
    _topic_argument(quality)
    quality.add_argument("--as-of")
    quality.add_argument("--output")
    quality.add_argument("--require-gates", action="store_true")
    quality.set_defaults(func=cmd_quality)
    report = sub.add_parser("report-check")
    _report_arguments(report)
    report.add_argument("--require-gates", action="store_true")
    report.set_defaults(func=cmd_report)
    init = sub.add_parser("audit-init")
    _report_arguments(init)
    init.set_defaults(func=cmd_audit_init)
    mechanical = sub.add_parser("audit-mechanical")
    _report_arguments(mechanical)
    mechanical.set_defaults(func=cmd_audit_mechanical)
    validate = sub.add_parser("audit-validate")
    validate.add_argument("--audit", required=True)
    validate.add_argument("--final", action="store_true")
    validate.set_defaults(func=cmd_audit_validate)
    return value
