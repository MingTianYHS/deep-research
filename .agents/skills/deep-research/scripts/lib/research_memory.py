...
MAX_DELTA_ITEM_CHARS = 500
MAX_BACKLOG_QUESTION_CHARS = 500
MAX_BACKLOG_REASON_CHARS = 500
MAX_BACKLOG_GAP_TYPE_CHARS = 120
MAX_CRITERIA = 5
MAX_CRITERION_CHARS = 500
MAX_KNOWN_EVIDENCE = 8
...
VALID_VERIFICATION_STATUSES = {"unchanged", "revalidated"}
VALID_VERIFICATION_RESULTS = {"confirmed", "content_changed"}
...
def _tokens(value: Any) -> set[str]:
    ...
    result = set(re.findall(r"[a-z0-9][a-z0-9._-]+", text))
    han = "".join(re.findall(r"[\u4e00-\u9fff]", text))
    result.update(han[index:index + 2] for index in range(max(0, len(han) - 1)))
    return result
...
def _usable_verification(item: dict[str, Any], attempts: dict[str, dict[str, Any]]) -> bool:
    attempt_id = str(item.get("source_attempt_id") or "")
    content_sha256 = str(item.get("content_sha256") or "")
    attempt = attempts.get(attempt_id)
    if not attempt or attempt.get("status") != "accepted" or attempt.get("eligible_for_evidence") is not True:
        return False
    if not content_sha256 or attempt.get("content_sha256") != content_sha256:
        return False
    if item.get("status") in VALID_VERIFICATION_STATUSES:
        return True
    return item.get("result") in VALID_VERIFICATION_RESULTS and item.get("statement_still_supported") is True
...
def latest_verifications(root: Path) -> dict[str, dict[str, Any]]:
    ...
    for _, item in iter_jsonl(root / "evidence/verifications.jsonl"):
        ...
        if evidence_id and _usable_verification(item, attempts):
            values[str(evidence_id)] = item
    return values


def _freshness(card: dict[str, Any], now: datetime, verification: dict[str, Any] | None = None) -> str:
    source = card.get("source") or {}
    latest = verification or {}
    checked = _iso(latest.get("verified_at") or source.get("last_verified_at") or source.get("accessed_at") or card.get("ingested_at"))
    if checked is None:
        return "unknown"
    limit = FRESH_DAYS.get(str(source.get("source_type") or "unknown"), FRESH_DAYS["unknown"])
...
def evidence_freshness(root: Path, card: dict[str, Any], now: datetime | None = None, verifications: dict[str, dict[str, Any]] | None = None) -> str:
    effective = verifications if verifications is not None else latest_verifications(root)
    return _freshness(card, now or datetime.now(timezone.utc), effective.get(str(card.get("id"))))
...
def _bounded_text(value: Any, maximum: int, label: str, errors: list[str]) -> None:
    if not isinstance(value, str) or not value.strip():
...
def validate_knowledge_delta(value: Any) -> list[str]:
    ...
    for key in DELTA_FIELDS:
        ...
        for index, item in enumerate(items, 1):
            _bounded_text(item, MAX_DELTA_ITEM_CHARS, f"knowledge_delta.{key}[{index}]", errors)
    return errors
...
def validate_next_research(value: Any, known_evidence: set[str] | None = None) -> list[str]:
    ...
    for index, item in enumerate(value, 1):
        ...
        if not isinstance(item_id, str) or not SAFE_ID.fullmatch(item_id):
            ...
            seen.add(item_id)
        _bounded_text(item.get("question"), MAX_BACKLOG_QUESTION_CHARS, f"next_research[{index}].question", errors)
        _bounded_text(item.get("reason"), MAX_BACKLOG_REASON_CHARS, f"next_research[{index}].reason", errors)
        _bounded_text(item.get("gap_type"), MAX_BACKLOG_GAP_TYPE_CHARS, f"next_research[{index}].gap_type", errors)
        ...
        if not isinstance(criteria, list) or not criteria:
            ...
            if len(criteria) > MAX_CRITERIA:
                errors.append(f"next_research[{index}].acceptance_criteria allows at most {MAX_CRITERIA} items")
            for criterion_index, criterion in enumerate(criteria, 1):
                _bounded_text(criterion, MAX_CRITERION_CHARS, f"next_research[{index}].acceptance_criteria[{criterion_index}]", errors)
        evidence_ids = item.get("known_evidence_ids")
        if not isinstance(evidence_ids, list):
            errors.append(f"next_research[{index}].known_evidence_ids must be a list")
        else:
            normalized = list(map(str, evidence_ids))
            if len(normalized) > MAX_KNOWN_EVIDENCE:
                errors.append(f"next_research[{index}].known_evidence_ids allows at most {MAX_KNOWN_EVIDENCE} items")
            if len(normalized) != len(set(normalized)):
                errors.append(f"next_research[{index}].known_evidence_ids must not contain duplicates")
            if known_evidence is not None:
                unknown = sorted(set(normalized) - known_evidence)
                if unknown:
...
def _select_cards(all_cards: list[dict[str, Any]], question_id: str | None, question_text: str | None, claims: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    if not question_id and not question_text:
        by_id = {str(item.get("id")): item for item in all_cards if item.get("id")}
        ...
        for claim in sorted(claims.values(), key=_claim_priority, reverse=True):
            for relation in claim.get("relations", []):
                ...
                if evidence_id in by_id and evidence_id not in seen:
                    selected.append(by_id[evidence_id]); seen.add(evidence_id)
                if len(selected) >= MAX_EVIDENCE:
                    return selected
        for card in reversed(all_cards):
            evidence_id = str(card.get("id") or "")
            if evidence_id and evidence_id not in seen:
                selected.append(card); seen.add(evidence_id)
            if len(selected) >= MAX_EVIDENCE:
                break
        return selected
    target = _tokens(question_text)
    ranked: list[tuple[int, int, dict[str, Any]]] = []
    for index, card in enumerate(all_cards):
        same = card.get("question_id") == question_id
        score = (4 if same else 0) + len(target & _tokens(card.get("statement")))
        if same or score > 0:
            ranked.append((score, index, card))
    ranked.sort(key=lambda item: (item[0], item[1]), reverse=True)
    return [item[2] for item in ranked[:MAX_EVIDENCE]]
...
def _select_queries(root: Path, question_id: str | None, question_text: str | None) -> list[dict[str, Any]]:
    ...
    for worker in _workers(root):
        for query in worker.get("queries_run", []) if isinstance(worker.get("queries_run"), list) else []:
            if not isinstance(query, dict) or not str(query.get("query") or "").strip():
                continue
            same = worker.get("question_id") == question_id
            score = (4 if same else 0) + len(target & _tokens(query.get("query")))
            candidates.append((score, position, worker, query)); position += 1
    ...
    for _, _, worker, query in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        selected.append({"query": _clip(query.get("query"), 300), "intent": query.get("intent"), "provider": query.get("provider"), "time_anchor": query.get("time_anchor"), "outcome": query.get("outcome"), "run_id": worker.get("run_id"), "question_id": worker.get("question_id")})
...
def build_reuse_plan(root: Path, question_id: str | None = None, question_text: str | None = None) -> dict[str, Any]:
    ...
    cards = _select_cards(all_cards, question_id, question_text, claims)
    ...
    for claim in claims.values():
        ...
        if evidence_ids.intersection(linked):
            relevant_claims.append({"id": claim.get("id"), "text": _clip(claim.get("text"), 300), "status": claim.get("status"), "confidence": claim.get("confidence"), "is_core": bool(claim.get("is_core")), "updated_at": claim.get("updated_at"), "evidence_ids": sorted(evidence_ids.intersection(linked))})
    ...
    for card in cards:
        ...
        if not url:
            continue
        verification = verifications.get(str(card.get("id")), {})
        freshness = _freshness(card, now, verification)
        freshness_values.append(freshness)
        attempt_id = verification.get("source_attempt_id") or card.get("source_attempt_id")
        ...
        origin_question = str(card.get("question_id") or "")
        if origin_question and origin_question not in existing["covered_questions"]:
            existing["covered_questions"].append(origin_question)
