...
def worker(worker_id: str, run_id: str, source_id: str, evidence_id: str, url: str, fact: str, group: str, digest: str) -> dict:
    ...
    return {
        "worker_result_version": 2, "worker_result_id": worker_id, "run_id": run_id, "status": "complete", "question_id": "q-001", "overlap_key": "smoke-boundary", "budget_profile": "lite", "coverage_status": "sufficient",
        "queries_run": [
        ...
        "rejected_sources": [], "contradictions": [], "gaps": [], "suggested_followups": [],
        "budget_used": {"tool_calls": 2, "search_queries": 2, "source_pages": 1, "duration_minutes": 1, "same_url_attempts_max": 1, "output_reserve_ratio": 0.25}, "stop_reason": "acceptance_criteria_met",
    }
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="deep-research-lifecycle-") as temporary:
        ...
        run(env, RESEARCH, "init-topic", "Lifecycle Smoke", "--slug", "lifecycle-smoke", "--budget", "lite")
        run(env, RESEARCH, "plan", "lifecycle-smoke", "--questions", "1")
        ...
        run(env, RESEARCH, "plan", "lifecycle-smoke")
        run_id = run(env, RESEARCH, "run-start", "lifecycle-smoke", "--mode", "baseline")["run_id"]
        for value in [worker("worker-one", run_id, "src-1", "ev-1", "https://example.com/one", "Lifecycle fact one", "origin-one", "a" * 64), worker("worker-two", run_id, "src-2", "ev-2", "https://example.org/two", "Lifecycle fact two", "origin-two", "b" * 64)]:
            path = topic / f"{value['worker_result_id']}.json"; path.write_text(json.dumps(value), encoding="utf-8"); run(env, RESEARCH, "ingest-worker", "lifecycle-smoke", "--file", path)
        state = json.loads((topic / "state.json").read_text(encoding="utf-8")); assert state["usage"]["queries"] == 4 and state["usage"]["pages"] == 2 and state["usage"]["evidence_cards"] == 2
        critic = {"id": "critic-lifecycle", "run_id": run_id, "reviewed_by": "research_critic", "status": "approved", "findings": [], "targeted_searches": [], "unresolved": [], "stop_reason": "review_complete"}; critic_path = topic / "critic.json"; critic_path.write_text(json.dumps(critic), encoding="utf-8"); run(env, RESEARCH, "critic-save", "lifecycle-smoke", "--file", critic_path)
        report = topic / "reports/final.md"; report.write_text(report_text(), encoding="utf-8")
        run(env, RESEARCH, "verify-citations", "lifecycle-smoke", "--report", report)
        ...
        finish = run(env, RESEARCH, "run-finish", "lifecycle-smoke", "--status", "complete"); assert finish["completion_gates"]["valid"]
        reflection = {"run_id": run_id, "critic_review_id": "critic-lifecycle", "summary": "Lifecycle completed", "open_questions": [], "next_actions": [], "lesson_candidates": []}; reflection_path = topic / "reflection.json"; reflection_path.write_text(json.dumps(reflection), encoding="utf-8"); run(env, RESEARCH, "reflect", "lifecycle-smoke", "--file", reflection_path)
        run(env, RESEARCH, "validate", "lifecycle-smoke")
