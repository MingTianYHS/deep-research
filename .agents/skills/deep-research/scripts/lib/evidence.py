...
def _append_missing(path: Path, records: list[dict[str, Any]], key: str = "id") -> None:
    existing = {str(item.get(key)): item for _, item in iter_jsonl(path) if item.get(key)}
    ...
    for item in records:
        ...
        if stored is not None:
            if _semantic_record(stored) != _semantic_record(item):
                raise ValueError(f"{path.name} {key} conflict: {item_id}")
            continue
...
def _resume_pending(topic_root: Path, pending_path: Path, result: dict[str, Any], max_new: int) -> dict[str, Any]:
    ...
    if pending.get("input_sha256") != _result_digest(result):
        raise ValueError(f"pending worker transaction differs from retry: {result.get('worker_result_id')}")
    accepted_cards = pending.get("accepted_cards", [])
    if not isinstance(accepted_cards, list) or len(accepted_cards) > max_new:
        raise ValueError("pending worker transaction exceeds current Evidence budget")
    _append_missing(topic_root / "logs/source_attempts.jsonl", pending.get("source_attempts", []))
    _append_missing(topic_root / "evidence/cards.jsonl", accepted_cards)
    _append_missing(topic_root / "evidence/verifications.jsonl", pending.get("verifications", []))
    worker_path = topic_root / "logs/workers" / f"{result['worker_result_id']}.json"
    atomic_write_json(worker_path, pending["logged"])
    pending_path.unlink(missing_ok=True)
    outcome = dict(pending["outcome"])
    outcome["recovered_transaction"] = True
    return outcome
...
def _verification(worker_id: str, existing: dict[str, Any], refreshed: dict[str, Any], attempt: dict[str, Any], prior_attempts: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
    if attempt.get("status") != "accepted" or not attempt.get("content_sha256"):
        return None
    original = prior_attempts.get(str(existing.get("source_attempt_id")), {})
    old_hash = original.get("content_sha256")
    new_hash = attempt.get("content_sha256")
    ...
    return {
        ...
        "verified_at": attempt.get("attempted_at") or utc_now(),
        "content_sha256": new_hash,
        "status": "unchanged" if old_hash and old_hash == new_hash else "revalidated",
        "statement_sha256": hashlib.sha256(str(refreshed.get("statement") or "").encode()).hexdigest(),
    }


def ingest_worker_result(cards_path: Path, result: dict[str, Any], max_new: int) -> dict[str, Any]:
    topic_root = cards_path.parent.parent
    worker_id = result.get("worker_result_id")
    pending_path = topic_root / "logs/workers" / f".{worker_id}.pending.json"
    ...
    if pending_exists:
        return _resume_pending(topic_root, pending_path, result, max_new)
    ...
    for attempt in source_attempts:
        if isinstance(attempt, dict):
            ...
            stored = stored_attempts.get(attempt_id)
            if stored is not None and _semantic_record(stored) != _semantic_record(attempt):
                raise ValueError(f"Source Attempt id conflict: {attempt_id}")
    ...
    for raw in cards:
        ...
        card_id = str(card["id"])
        key = evidence_key(card)
        existing_for_id = existing_by_id.get(card_id)
        if existing_for_id is not None and evidence_key(existing_for_id) != key:
            raise ValueError(f"Evidence id conflict: {card_id}")
        if card_id in existing_ids or key in existing_keys:
            existing = existing_for_id or existing_by_key.get(key)
            duplicate_ids.append(str(existing.get("id") if existing else card_id))
            if existing:
                ...
                if event:
                    ...
                    verified_ids.append(str(existing["id"]))
            continue
        existing_ids.add(card_id)
        existing_keys.add(key)
        existing_by_id[card_id] = card
    ...
    logged["ingest_summary"] = {
        "accepted_evidence_ids": accepted_ids,
        "new_evidence_ids": [card["id"] for card in accepted],
    ...
    outcome = {
        ...
        "new_ids": [card["id"] for card in accepted],
        ...
        "ingest_context_validation": context_validation,
        "worker_result_log": str(worker_path),
    }
    ...
    _append_missing(topic_root / "logs/source_attempts.jsonl", source_attempts)
    _append_missing(cards_path, accepted)
    _append_missing(topic_root / "evidence/verifications.jsonl", verifications)
...
def _validate_ingest_integrity(result: dict[str, Any]) -> None:
    ...
    for index, attempt in enumerate(result.get("source_attempts", []), 1):
        ...
        try:
            normalized = normalize_url(str(attempt.get("url", "")))
        except ValueError as exc:
            errors.append(f"source_attempts[{index}] invalid url: {exc}")
            continue
        if attempt.get("normalized_url") != normalized:
            errors.append(f"source_attempts[{index}] normalized_url does not match url")
        if attempt.get("status") == "accepted" and not SHA256_RE.fullmatch(str(attempt.get("content_sha256", ""))):
            errors.append(f"source_attempts[{index}] content_sha256 must be 64 lowercase hexadecimal characters")
    for index, card in enumerate(result.get("evidence_cards", []), 1):
        if not isinstance(card, dict):
            continue
        attempt = attempts.get(str(card.get("source_attempt_id", "")))
        source = card.get("source") or {}
        if not attempt or not isinstance(source, dict):
            continue
        try:
            source_url = normalize_url(str(source.get("url", "")))
        except ValueError as exc:
            errors.append(f"evidence_cards[{index}] invalid source.url: {exc}")
            continue
        if source_url != attempt.get("normalized_url"):
            errors.append(f"evidence_cards[{index}] source.url does not match its Source Attempt")
    usage = result.get("budget_used") or {}
    queries = result.get("queries_run", [])
    source_attempts = result.get("source_attempts", [])
    if usage.get("search_queries") != len(queries):
        errors.append("budget_used.search_queries must equal queries_run count")
    if usage.get("source_pages") != len(source_attempts):
        errors.append("budget_used.source_pages must equal source_attempts count")
    if errors:
        raise ValueError("invalid worker source integrity: " + "; ".join(sorted(set(errors))))
