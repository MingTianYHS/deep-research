...
def _worker_version(result: dict[str, Any], errors: list[str], warnings: list[str]) -> int:
    version = result.get("worker_result_version", 1)
    if not isinstance(version, int) or isinstance(version, bool) or version not in {1, 2}:
        errors.append("worker_result_version must be 1 or 2")
        return 1
    if version == 1:
        warnings.append("legacy worker query trace; migrate to worker_result_version 2")
    return version


def _validate_queries(result: dict[str, Any], version: int, errors: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for index, query in enumerate(values, 1):
        if not isinstance(query, dict):
            errors.append(f"queries_run[{index}] must be an object for worker_result_version 2")
            continue
        ...
        if isinstance(text, str) and text.strip():
            normalized.setdefault(_normalized_query(text), []).append(query)
    for text, matches in normalized.items():
        providers = {str(item.get("provider")) for item in matches}
        if len(providers) > 1:
...
def _validate_attempts(result: dict[str, Any], version: int, queries: dict[str, dict[str, Any]], errors: list[str]) -> dict[str, dict[str, Any]]:
    attempts: dict[str, dict[str, Any]] = {}
    for index, attempt in enumerate(result.get("source_attempts", []) if isinstance(result.get("source_attempts"), list) else [], 1):
...
def _validate_cards(result: dict[str, Any], attempts: dict[str, dict[str, Any]], errors: list[str]) -> None:
    for index, card in enumerate(result.get("evidence_cards", []) if isinstance(result.get("evidence_cards"), list) else [], 1):
...
def validate_worker_result(result: dict[str, Any], limits: dict[str, Any]) -> dict[str, Any]:
    ...
    version = _worker_version(result, errors, warnings)
    queries = _validate_queries(result, version, errors)
    attempts = _validate_attempts(result, version, queries, errors)
    ...
    if version == 2 and result.get("status") == "complete":
        if not queries:
            errors.append("complete version-2 worker result requires query trace")
        if not any(query.get("intent") == "disconfirming" for query in queries.values()):
            errors.append("complete version-2 worker result requires a disconfirming query")
    ...
    if version == 2 and low_yield_fallback:
    ...
    return {"valid": not errors, "errors": sorted(set(errors)), "warnings": sorted(set(warnings)), "limits": limits, "budget_verification": "self_reported", "worker_result_version": version}
