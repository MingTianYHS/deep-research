...
def materialize(path: Path) -> dict[str, dict[str, Any]]:
    ...
    for _, event in iter_jsonl(path):
        ...
        if event_type == "claim.created":
            ...
            relations = claims[claim_id].setdefault("relations", [])
            relations = [item for item in relations if item.get("evidence_id") != relation["evidence_id"]]
            ...
            claims[claim_id]["pending_transition"] = {
                "to": event["to"], "reason": event.get("reason", ""), "proposed_at": event["at"]
            }
...
def link(path: Path, claim_id: str, evidence_id: str, stance: str, strength: float) -> dict[str, Any]:
    ...
    event = {
        "type": "claim.relation.added",
        "claim_id": claim_id,
        "relation": {"evidence_id": evidence_id, "stance": stance, "strength": strength},
        "at": utc_now(),
    }
...
def change_status(path: Path, claim_id: str, target: str, reason: str, approve_core: bool) -> dict[str, Any]:
    ...
    if claim.get("is_core") and not approve_core:
