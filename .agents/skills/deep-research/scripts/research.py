...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise SystemExit("internal validation returned invalid JSON") from exc
    structural["topic"] = naming["topic"]
    structural["workspace"] = naming["workspace"]
    print(json.dumps(structural, ensure_ascii=False, indent=2))
    if exit_code or not structural.get("valid", False):
...
def _topic_argument(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "topic",
        nargs="?",
        help="Topic directory/name. Omit inside a topic workspace.",
    )
...
def parser() -> argparse.ArgumentParser:
    ...
    plan = sub.add_parser(
        "plan", help="Create or synchronize the canonical Research Design."
    )
    ...
    brief = sub.add_parser(
        "brief", help="Build bounded context for the topic or one question."
    )
    ...
    start = sub.add_parser(
        "start", help="Start a baseline, incremental, or deep-dive Run."
    )
    ...
    finish = sub.add_parser(
        "finish", help="Close the active Run after all required gates."
    )
    _topic_argument(finish)
    finish.add_argument(
        "--status", choices=["complete", "partial", "failed"], default="complete"
    )
    finish.add_argument("--note", default="")
    finish.set_defaults(func=cmd_finish)

    validate = sub.add_parser(
        "validate", help="Validate naming, workspace, design, and records."
    )
