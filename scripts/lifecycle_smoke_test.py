...
def worker(
    ...
    return {
        ...
        "status": "complete",
        "question_id": "q-001",
        "overlap_key": "smoke-boundary",
        "budget_profile": "lite",
        "coverage_status": "sufficient",
        "queries_run": [
            {
                "id": f"query-{prefix}-primary",
                "query": fact,
                "intent": "primary_source",
                "provider": "native_web",
                "language": "en",
                "time_anchor": "2026-08-03",
                "fallback_of": None,
                "outcome": "primary_candidate_found",
            },
            {
                "id": f"query-{prefix}-against",
                "query": fact + " limitations",
                "intent": "disconfirming",
                "provider": "native_web",
                "language": "en",
                "time_anchor": "2026-08-03",
                "fallback_of": None,
        ...
        "source_attempts": [
            {
                ...
                "query_id": f"query-{prefix}-primary",
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="deep-research-lifecycle-") as temporary:
        ...
        run_id = run(
            env, PUBLIC, "start", "lifecycle-smoke", "--mode", "baseline"
        )["run_id"]
        ...
        for item in audit["items"]:
            item.update(
                ...
                checked_by="research_critic",
                observed_text=item["expected_quote"],
        ...
        finish = run(
            env, PUBLIC, "finish", "lifecycle-smoke", "--status", "complete"
        )
        assert finish["completion_gates"]["valid"]

        reflection = {
            "run_id": run_id,
            "critic_review_id": "critic-lifecycle",
