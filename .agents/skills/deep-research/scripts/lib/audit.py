...
ALLOWED_STATUSES={"pending","verified","failed","unavailable"};MATCH_TYPES={"exact","normalized","semantic","locator_only"}
...
def validate_audit(path:Path,require_all_verified:bool=False)->dict[str,Any]:
 ...
 for index,item in enumerate(items,1):
  ...
  if not evidence_id:errors.append(f"item {index}: missing evidence_id")
  elif evidence_id in seen:errors.append(f"item {index}: duplicate evidence_id {evidence_id}")
  seen.add(evidence_id);status=item.get("status")
