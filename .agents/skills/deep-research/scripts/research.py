...
def cmd_next(args: argparse.Namespace) -> None:
    ...
    if lease is not None:
        result["coordinator_lease"] = lease
    if run_id:
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    if not naming["valid"]:
        result = {"valid": False, "topic": naming["topic"], "workspace": naming["workspace"], "errors": naming["errors"], "warnings": []}
    ...
    structural["topic"] = naming["topic"]; structural["workspace"] = naming["workspace"]
    print(json.dumps(structural, ensure_ascii=False, indent=2))
