...
def test_audit_requires_observed_source_proof(tmp_path):
    report=tmp_path/"report.md"; report.write_text("Fact [[ev-1]]"); output=tmp_path/"audit.json"; create_audit(report,{"ev-1":card()},output)
    audit=json.loads(output.read_text()); audit["items"][0].update(status="verified",checked_at="2026-08-01T00:00:00Z",checked_by="codex/reviewer") ; atomic_write_json(output,audit); assert not validate_audit(output,True)["valid"]
