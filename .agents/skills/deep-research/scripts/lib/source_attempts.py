...
def may_attempt(path: Path, url: str, max_attempts: int = 2) -> dict[str, Any]:
    normalized = normalize_url(url); attempts = [item for item in load_attempts(path) if item.get("normalized_url") == normalized]; accepted = next((item for item in attempts if item.get("status") == "accepted"), None)
...
def build_attempt(url: str, tool: str, http_status: int | None, content: str, *, source_version: str | None = None, access_mode: str = "public_static") -> dict[str, Any]:
    assessment = assess_response(http_status, content)
    return {"id": f"src-{uuid.uuid4().hex[:12]}", "url": url, "normalized_url": normalize_url(url), "tool": tool, "access_mode": access_mode, "http_status": http_status, "source_version": source_version, **assessment}


def append_attempt(path: Path, attempt: dict[str, Any]) -> dict[str, Any]:
    existing = load_attempts(path); content_hash = attempt.get("content_sha256")
    if content_hash:
