...
def test_public_commands_are_small_and_workflow_focused():
    ...
    assert set(subparsers.choices) == {"new", "plan", "brief", "start", "status", "report", "finish", "validate"}


def test_new_always_uses_guarded_topic_controller(monkeypatch):
    captured = {}
    monkeypatch.setattr(module.topicctl, "cmd_init", lambda args: captured.update(vars(args)))
...
def test_report_always_uses_guarded_topic_controller(monkeypatch):
    captured = {}
    monkeypatch.setattr(module.topicctl, "cmd_report", lambda args: captured.update(vars(args)))
    module.cmd_report(argparse.Namespace(topic=None, type="final", title=None, output=None, allow_language_mismatch=True))
...
def test_language_mismatch_override_is_available_for_lifecycle_commands():
    report = module.parser().parse_args(["report", "topic", "--allow-language-mismatch"])
    validate = module.parser().parse_args(["validate", "topic", "--allow-language-mismatch"])
