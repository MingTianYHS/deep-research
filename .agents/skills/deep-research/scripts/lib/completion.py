...
def _quality(root: Path, cards: list[dict[str, Any]], policy_file: Path, covered_question_ids: set[str], run_id: str | None = None) -> dict[str, Any]:
    ...
    question_ids = _scoped_question_ids(root, str(run_id or read_json(root / "state.json", {}).get("active_run_id") or ""))
    covered = question_ids.intersection(covered_question_ids)
    ...
    result["scoring_mode"] = "hard_gates_only"
    return result
...
def completion_gate(root: Path, run_id: str, skill_dir: Path) -> dict[str, Any]:
    ...
    covered_question_ids = {str(worker.get("question_id")) for worker in workers if worker.get("status") == "complete" and (worker.get("ingest_summary") or {}).get("accepted_evidence_ids")}
    expected_questions = _scoped_question_ids(root, run_id)
    outside = covered_question_ids - expected_questions
    if outside:
        errors.append(f"Worker Results cover questions outside the active run scope: {sorted(outside)}")
    missing = expected_questions - covered_question_ids
    if missing:
        errors.append(f"complete run requires every assigned question; missing: {sorted(missing)}")
    evidence = _evidence(root)
    run_cards = [card for evidence_id, card in evidence.items() if evidence_id in accepted_evidence_ids]
    if not run_cards:
        errors.append("complete run requires Evidence accepted or explicitly reused during the active run")
    ...
    if not quality["passes_all_gates"]:
        errors.append("complete run requires all active-run Evidence hard gates")
    report_candidates = []
    rubric = load_rubric(skill_dir / "config/report_rubric.toml")
    for audit_path in sorted((root / "reports").glob("*.md.audit.json")):
        audit = read_json(audit_path, {})
        if audit.get("run_id") != run_id or not audit.get("report"):
            continue
    ...
    if not passing:
        errors.append("complete run requires a current-run citation-valid report that passes hard report gates and the profile audit")
